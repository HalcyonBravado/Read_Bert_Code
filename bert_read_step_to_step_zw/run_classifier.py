...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.max_steps > 0:  # 梯度累计，总训练步数
    ...
    if args.local_rank != -1:  # 为各个进程分配rank号
    ...
    model.zero_grad()
    seed_everything(args.seed)  # 初始化Added here for reproductibility (even between python 2 and 3)
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            outputs = model(**inputs)  # 包裹传递方式，不确定参数个数，*tuple **dict
